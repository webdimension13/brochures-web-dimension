export default async function handler(req, res) {
  ...
  try {
    ...
    // Filtre strict : le mot-clé doit contenir tous les mots du seed le plus pertinent
    const terms = [metier, service1, service2].filter(Boolean).map(t => t.toLowerCase().trim());

    const filteredSimilar = similarKeywords
      .filter(kw => {
        const kwL = kw.toLowerCase();
        // Doit commencer par l'un des termes
        if (!terms.some(t => kwL.startsWith(t))) return false;
        // Tous les mots du terme le plus proche doivent être présents dans le mot-clé
        return terms.some(t => {
          const tWords = t.split(' ');
          return tWords.every(w => kwL.includes(w));
        });
      })
    ...
    return res.status(200).json({
      ...
      total_similar: filteredSimilar.length,
